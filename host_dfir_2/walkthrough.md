...
Before starting, ensure the Splunk container is running by executing the following command in your terminal:
```bash
./host_dfir_2/scenario.sh start
```
...
### 9. Cleanup
Once you have documented all findings, stop the environment:
```bash
./host_dfir_2/scenario.sh stop
```